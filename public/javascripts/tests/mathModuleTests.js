...
QUnit.test("when high distribution rate, check if generated array has normal distributed numbers", function(assert) {
    var max = 20000;
    
    arr = wordCloud.mathModule.generateArrayWithNormalDistributedElements(max);
    
    assert.ok(arr[2] >= arr[1] && arr[1] >= arr[0], 
        "the first element is smaller than the second, the second smaller than the third.");
});